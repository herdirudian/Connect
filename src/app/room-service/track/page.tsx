...
import { Utensils, Brush, Copy } from 'lucide-react';
...
export default function TrackOrderPage() {
  ...
  const [filter, setFilter] = useState<'ALL' | 'FOOD' | 'HK'>('ALL');
  ...
  useEffect(() => {
    if (phone && last4.length === 4) {
      // Auto fetch when phone is present from redirect
      fetchOrder();
    }
  ...
  async function fetchOrder() {
    ...
    try {
      ...
      const res = await fetch(`/api/room-service/orders/by-phone${qs}`, { cache: 'no-store' });
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      <div className="space-y-2">
        <h1 className="text-2xl font-black text-brand-dark uppercase tracking-tight">Track Room Service Order</h1>
        <p className="text-gray-500">Masukkan nomor telepon tamu untuk melihat status pesanan.</p>
      </div>
        ...
        <CardContent className="p-4 space-y-3">
          <div className="grid sm:grid-cols-3 gap-3">
            <input
              ...
              placeholder="Nomor Telepon Tamu (lengkap)"
              value={phone}
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            ...
            <button
              className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border transition ${filter === 'ALL' ? 'bg-brand-50 text-brand-dark border-brand' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50'}`}
              onClick={() => setFilter('ALL')}
            >
              Semua
            </button>
            <button
              className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border transition ${filter === 'FOOD' ? 'bg-brand-50 text-brand-dark border-brand' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50'}`}
              onClick={() => setFilter('FOOD')}
            >
              Food
            </button>
            <button
              className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border transition ${filter === 'HK' ? 'bg-brand-50 text-brand-dark border-brand' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50'}`}
        ...
        <div className="space-y-4">
          {orders.length === 0 ? (
            ...
            orders
              ...
              return (
                <Card key={order.id} className={`border-2 rounded-2xl ${order.type === 'HK' ? 'border-yellow-200' : 'border-green-200'} hover:shadow-md transition`}>
                  <CardHeader>
                    <CardTitle className="text-lg flex items-center gap-2">
                      {order.type === 'HK' ? <Brush className="h-4 w-4 text-yellow-600" /> : <Utensils className="h-4 w-4 text-green-600" />}
                  ...
                  <CardContent className="space-y-3">
                    <div className="text-sm text-gray-700">
                      ...
                      {order.type === 'FOOD' && (
                        <div>Metode: {order.deliveryNotes === 'DELIVER_TO_ROOM' ? 'Diantar ke Kamar' : order.deliveryNotes === 'DINE_AT_CAFE' ? 'Makan di Cafe' : '-'}</div>
                      )}
                    ...
                    <div className="flex flex-wrap gap-2 justify-end">
                      ...
                      {order.type === 'FOOD' && (
                        <>
                          ...
                          <Button
                            variant="outline"
                            onClick={async () => {
                              const email = prompt('Masukkan email tujuan invoice:');
                              if (!email) return;
                              try {
                                ...
                                if (res.ok) alert('Invoice terkirim');
                                else alert('Gagal mengirim invoice');
                              } catch {
                      ...
                      {order.type === 'HK' && (
                        <>
                          <Button variant="outline" onClick={() => window.open(`/api/housekeeping/orders/${order.id}/invoice`, '_blank')}>
                            Download Invoice
                          </Button>
                          <Button
                            variant="outline"
                            onClick={async () => {
                              ...
                              if (!email) return;
                              try {
                                const res = await fetch(`/api/housekeeping/orders/${order.id}/send-invoice`, {
                                  method: 'POST',
                                  headers: { 'Content-Type': 'application/json' },
                                  body: JSON.stringify({ to: email })
                                });
