...
import { Tent, Star, Users, Calendar } from 'lucide-react';
...
interface Accommodation {
  ...
  benefits: string;
  imageUrl?: string;
  images?: string[]; // Handle Json type
  active: boolean;
  availability?: number;
}
...
export default function StayPage() {
  ...
  useEffect(() => {
    fetchAccommodations();
  }, []); // Only fetch on mount (defaults to today)
  ...
  async function fetchAccommodations() {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        console.error('Invalid accommodations data:', data);
        setAccommodations([]);
      }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            ...
            <Link href="/dashboard/bookings" className="w-full sm:w-auto">
              <Button variant="outline" className="w-full shadow-sm bg-white">My Bookings</Button>
            </Link>
          </div>
        </div>
