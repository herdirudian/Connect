...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await transporter.sendMail({
      ...
      to: recipient,
      subject: `Invoice Housekeeping Order #${order.id.slice(0,8)} - The Lodge Maribaya`,
      html: `
        <div style="font-family: Arial,sans-serif; padding:16px;">
          <h2>Invoice Housekeeping Order</h2>
      ...
      `,
      attachments: [
        { filename: `invoice-housekeeping-${order.id}.pdf`, content: pdf }
      ]
    });
