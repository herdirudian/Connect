...
export async function GET(req: Request) {
  try {
    ...
    if (view === 'items') {
      ...
      for (const it of foodItems) addItem(it.order.createdAt, 'FOOD', it.menuItem?.name || 'Item', it.price || 0, it.quantity || 0);
      for (const it of hkItems) addItem(it.order.createdAt, 'HOUSEKEEPING', it.item?.name || 'Item', it.price || 0, it.quantity || 0);
      for (const b of bookingRows) {
        ...
        if (!addedAny) addItem(b.createdAt, 'BOOKING', `Booking ${b.type || ''}`.trim(), b.amount || 0, 1);
      }
      ...
      const header = ['Date', 'Source', 'Item', 'UnitPrice', 'Qty', 'Total'].join(',');
      const csvRows = rows.map((r) => [r.date, r.source, `"${String(r.item).replaceAll('"', '""')}"`, r.unitPrice, r.quantity, r.total].join(','));
      const csv = [header, ...csvRows].join('\n');

      return new NextResponse(csv, {
        headers: {
          'Content-Type': 'text/csv',
          'Content-Disposition': `attachment; filename="daily-sales-items-${Date.now()}.csv"`,
        },
      });
    }
    ...
    const map: Record<string, { bookings: number; food: number; housekeeping: number; total: number }> = {};
    const add = (dt: Date, amount: number, key: 'bookings' | 'food' | 'housekeeping') => {
      const d = new Date(dt);
      const dateKey = ymd(new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate())));
      if (!map[dateKey]) map[dateKey] = { bookings: 0, food: 0, housekeeping: 0, total: 0 };
    ...
    const header = ['Date', 'Bookings', 'Food', 'Housekeeping', 'Total'].join(',');
    const csvRows = rows.map(([date, v]) => [date, v.bookings, v.food, v.housekeeping, v.total].join(','));
    const csv = [header, ...csvRows].join('\n');
    return new NextResponse(csv, {
      headers: {
        'Content-Type': 'text/csv',
        'Content-Disposition': `attachment; filename="daily-sales-${Date.now()}.csv"`,
      },
    });
  } catch (e: any) {
