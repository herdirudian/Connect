...
export default function AdminChildrensDayBiodefPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/admin/childrens-day?sponsor=BIODEF');
  ...
  const handleSaveQuota = async () => {
    ...
    try {
      const res = await fetch('/api/admin/childrens-day', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ maxQuota: parseInt(tempQuota.toString(), 10), sponsor: 'BIODEF' })
      });
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-gray-900">Promo Hari Anak x Biodef</h2>
          <p className="text-muted-foreground">Manajemen pendaftar tiket gratis Hari Anak Nasional Sponsor Biodef.</p>
        </div>
