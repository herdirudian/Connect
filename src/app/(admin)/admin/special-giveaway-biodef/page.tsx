...
export default function AdminSpecialGiveawayBiodefPage() {
  const [data, setData] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [count, setCount] = useState(0);
  const [maxQuota, setMaxQuota] = useState(30);
  ...
  const [tempQuota, setTempQuota] = useState(30);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/admin/childrens-day?sponsor=GIVEAWAY_BIODEF');
  ...
  const handleSaveQuota = async () => {
    ...
    try {
      const res = await fetch('/api/admin/childrens-day', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ maxQuota: parseInt(tempQuota.toString(), 10), sponsor: 'GIVEAWAY_BIODEF' })
      });
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `data_giveaway_biodef_${new Date().getTime()}.csv`);
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-gray-900">30 Pemenang Special Giveaway</h2>
          <p className="text-muted-foreground">Manajemen pendaftar 30 Pemenang Special Giveaway Package x Biodef.</p>
        </div>
